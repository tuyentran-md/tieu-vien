...
- Day mood ramp: dawn (slightly cool, mist heavier) → day (clear) → after choice resolved: dusk (warm tint, window light stronger) → nút kết ngày fades to ink night, then next dawn. One overlay div with CSS transitions, ≥600ms.
...
scene.setPhase(phase);       // "day" | "result" (dusk tint) | "night" (fade khi kết ngày)
scene.onTap(cb);             // cb(hotspotId) — "porch"|"tree"|"gate"|"jar"|"npc"|item ids
scene.setHotspotsGlow(on);   // empty days: 3-4 soft glowing dots
```

### NPC painted figures (PNG, SVG fallback; one visible at a time)
...
Walk = CSS transform transition 6.4s ease-in-out (timer, không rAF — tab ẩn không kẹt), gate → giữa sân; tap bóng người đang đi = giục vào sân, mở lời ngay. Với `prefers-reduced-motion`, khách vào gần như tức thì.

### Yard items (`S.items`) — transparent PNG/SVG elements toggled visible
...
## 3. Flow (`boot.js` owns the current day-loop rendering)
...
- **Khép lại một ngày** → `scene.setPhase("night")` fade 600ms → `S.day++`, reset, dawn.
- **Epilogue** (DOM fullscreen): `C.epilogueParas(S)` + stats, snow drift behind, WebAudio ending, "Qua thêm một năm nữa" clears save.
...
Toàn bộ âm thanh sinh trong `audio.js`: ambient gió và nước từ filtered noise / mưa bandpass theo thời tiết / chim và các motif ngũ cung thưa. SFX: mõ gỗ (chọn), tick giấy (mở/gấp), hai nốt chuông penta (mở khóa Sổ Nhỏ / vật). Epilogue dùng một motif thưa riêng. Unlock sau first gesture; `tieuvien_sound` toggle persists.
...
- **N2 (wiring, codex):** boot day loop + dialog + epilogue + autoplay chạy full 48 ngày. Gate: sim green; ngày 1 mở được Mộc dialog với các choice hợp lệ; full run reaches epilogue.
